    ...
    public function updateProfile(Request $request)
{
    ...
    public function gallery(Request $request)
    {
        ...
        $galleryItemsWithUrls = $galleryItems->map(function ($item) {
            ...
            // Full image URLs
            $itemArray['image_urls'] = collect($item->image_paths)->map(function ($path) {
            ...
            // Full video URLs
            $itemArray['video_urls'] = collect($item->video_paths)->map(function ($path) {
            ...
            // Remove old fields
            unset($itemArray['image_paths']);
            unset($itemArray['video_paths']);
            unset($itemArray['image_paths_url']);
            unset($itemArray['video_paths_url']);
    ...
    public function event(Request $request)
    {
        ...
        // Get event items from same admin with RSVP status
        ...
        // Add full image URLs and RSVP status
        $eventsWithUrls = $events->map(function ($event) {

            $eventArray = $event->toArray();

            // Convert image_paths array into full URLs
            $eventArray['image_paths'] = collect($event->image_paths)->map(function ($path) {
                return asset('storage/' . $path);
            })->toArray();

            // Single image full URL
            $eventArray['image_path'] = !empty($event->image_paths[0])
                ? asset('storage/' . $event->image_paths[0])
                : null;
        ...
        // Upcoming events
        $upcomingEvents = $eventsWithUrls->filter(function ($event) use ($now) {

            return \Carbon\Carbon::parse($event['posted_date'])
                ->startOfDay()
                ->greaterThanOrEqualTo($now);
        })->values();

        // Past events
        $pastEvents = $eventsWithUrls->filter(function ($event) use ($now) {
    ...
    public function support(Request $request)
    {
        ...
        // Get support items from same admin
        ...
        // Add full APP_URL for image
        $supportsWithUrls = $supports->map(function ($support) {

            $supportArray = $support->toArray();

            // Full image URL
            $supportArray['image'] = !empty($support->image)
                ? asset('storage/' . $support->image)
                : null;

            // Optional extra field
            $supportArray['image_url'] = !empty($support->image)
                ? asset('storage/' . $support->image)
                : null;
        ...
        return response()->json([
            'status' => 'success',
            'data' => $supportsWithUrls
        ]);
    }

    /**
     * Display the QR Code and App URL for the customer
     */
    public function qrCode(Request $request)
    {
        $customer = Auth::guard('sanctum')->user();

        // Get QR code from same admin
        $qrCode = QrCode::where('admin_id', $customer->admin_id)
            ->latest()
            ->first();

        $qrCodeData = null;
        if ($qrCode) {
            $qrCodeData = $qrCode->toArray();
            $qrCodeData['image_url'] = $qrCode->image ? asset('storage/' . $qrCode->image) : null;
        }
        ...
        if ($categoryName) {
            ...
            $data = $category->supports->map(function ($support) use ($category) {
                ...
                return [
                    ...
                    // Full APP_URL for image
                    'image' => !empty($support->image)
                        ? asset('storage/' . $support->image)
                        : null,

                    // Extra image URL field
                    'image_url' => !empty($support->image)
                        ? asset('storage/' . $support->image)
                        : null,
